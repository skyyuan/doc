	...
	if item.Release != "" {
		...
		if docQuery, err := goquery.NewDocumentFromReader(bytes.NewBufferString(item.Release)); err == nil {
			...
			if selector := docQuery.Find("div.wiki-bottom").First(); selector.Size() <= 0 && item.MemberId > 0 {
				//处理文档结尾信息
				fmt.Println("fuck")
				fmt.Println(item.ModifyAt)
				fmt.Println("fuck nima ")
				hisdoc, _ := NewDocumentHistory().DocIdLast(item.DocumentId)
				docCreator, err := NewMember().Find(hisdoc.ModifyAt, "real_name", "account")
				release := "<div class=\"wiki-bottom\">文档更新时间: " + item.ModifyTime.Local().Format("2006-01-02 15:04") + " &nbsp;&nbsp;修改人："
