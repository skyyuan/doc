...
// 根据项目标识查询项目.
func (m *BookResult) FindByIdentifyNotUser(identify string, memberId int, cols ...string) (*BookResult, error) {
	if identify == "" || memberId <= 0 {
		return m, ErrInvalidParameter
	}
	o := orm.NewOrm()
	...
	err = o.QueryTable(relationship.TableNameWithPrefix()).Filter("book_id", book.BookId).One(relationship)
	if err != nil {
		return m, err
	}
	var relationship2 Relationship
